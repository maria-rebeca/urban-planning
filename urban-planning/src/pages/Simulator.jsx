...
  const [predictedPollution, setPredictedPollution] = useState(null); 

  const [replacementVictims, setReplacementVictims] = useState(['13300', '13400', '33000']); 
...
      
      const smartDist = calculateSmartDistribution(currentDist, targetCode, percentage, replacementVictims);
...
        const deltaTemp = newTemp - baselineTemp;

        // Poluarea 
        const baselinePollution = baselineResponse.data.predicted_pollution;
        const newPollution = newResponse.data.predicted_pollution;
        
        console.log(`📉 Baseline Temp: ${baselineTemp.toFixed(2)} | 📈 New Temp: ${newTemp.toFixed(2)} | Delta: ${deltaTemp.toFixed(4)}`);
        console.log(`📉 Baseline Pollut: ${baselinePollution.toFixed(2)} | 📈 New Pollut: ${newPollution.toFixed(2)}`);

  
        const realTemp = markerStats.target_temp || markerStats.mean_temp || 0;
        const finalPrediction = realTemp + deltaTemp;
        
        const finalPollution = newPollution; 

        setPrediction(finalPrediction);
        setPredictedPollution(finalPollution); 
      } else {
...
        alert("Failed to connect to AI server. Please ensure 'python app.py' is running on port 5000.");
      } else if (error.message) {
        alert("Simulation Error (Client Side). Check console for details. Message: " + error.message);
      } else {
        alert("Simulation Error: Unknown error occurred.");
      }
    } finally {
      setIsLoading(false);
    }
...
      alert("Error fetching map stats. Ensure backend is running and GEE is initialized.");
    } finally {
      setIsLoading(false);
    }
  }, []);
...
               *Simulated by replacing selected victim land first.
            </div>
          </div>
        )}
      </div>
